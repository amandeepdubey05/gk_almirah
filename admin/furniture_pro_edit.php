          ...
          <form method="post" enctype="multipart/form-data">
             <?php
                    if(isset($_GET['pid'])){
                      ...
                      $Fur_pr_query = "SELECT * FROM furniture_product WHERE product_id=$Fur_pro_id";
                      $Fur_pr_run   = mysqli_query($con,$Fur_pr_query);

                      if(mysqli_num_rows($Fur_pr_run) > 0){
                         $Fur_pr_row = mysqli_fetch_array($Fur_pr_run);
                          $db_pid   = $Fur_pr_row['product_id'];
                          $db_title = $Fur_pr_row['product_name'];
                          $db_category = $Fur_pr_row['product_cat'];
                          $db_size = $Fur_pr_row['product_size'];
                          $db_price = $Fur_pr_row['product_price'];    
                          $db_detail  = $Fur_pr_row['product_desc'];
                          $db_image = $Fur_pr_row['product_img1'];
                          ...
                          if(isset($_POST['update'])){ 
                            ...
                            if(!empty($title) || !empty($size) || !empty($price) || !empty($status) || !empty($category) || !empty($detail) || !empty($image)){
                              ...
                              $query = "UPDATE furniture_product SET product_title='$title', product_cat=$category, product_size='$size', product_price =$price, product_desc='$detail', product_img1='$image', status='$status' Where product_id=$Fur_pro_id";                            
